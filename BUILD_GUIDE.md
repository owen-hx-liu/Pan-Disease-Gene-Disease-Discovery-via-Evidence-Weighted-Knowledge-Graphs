...
existing script), in dependency order. For each: what it is, which machine owns it, inputs →
...
```
STAGE 1  backbone (Machine C)         STAGE 2  methods            STAGE 3  method + validation
  1. graph_stats.py .................. 4. lib_eval.py (C) ........ 7. build_hybrid.py (A+B)
  2. build_deleaked_splits.py [DONE].. 5. run_baselines.py (B) ... 8. loso_validation.py (B)
  3. provenance fill (data) .......... 6. run_kge.py (A) ......... 9. newer_release_diff.py (C)
                                                                  10. literature_curate.py (C)
STAGE 4  outputs (Machine C)          STAGE 5  release (Machine C)
 11. make_figures.py                   13. run_all.ps1 + Zenodo
 12. make_tables.py                    14. manuscript
```

Nothing in Stage 2 can start until Unit 4 (`lib_eval.py`) and the split files (Unit 2) exist
and are hashed. That is the whole reason Unit 4 is owned by C and shared.

---

# STAGE 1 — Backbone (Machine C, do first)

### Unit 1 — `scripts/graph_stats.py`  `NEW` · Machine C
...
### Unit 2 — `scripts/build_deleaked_splits.py`  `DONE` · Machine C
...
### Unit 3 — Provenance finalize  `EXTEND`/data · Machine C
...
### Unit 4 — `scripts/lib_eval.py`  `NEW` · Machine C (shared library — the linchpin)
- **Purpose:** the single implementation of the ranking/metric protocol that **both**
  ...
  One owner (C) so it never diverges.
...
### Unit 5 — `scripts/run_baselines.py`  `NEW` (may fold in `bench_baselines.py`) · Machine B
...
### Unit 6 — `scripts/run_kge.py`  `EXTEND scripts/kge_benchmark.py` · Machine A (GPU)
...
### Unit 7 — `scripts/build_hybrid.py`  `NEW` · Machine A leads, B supplies candidates
...
### Unit 8 — `scripts/loso_validation.py`  `NEW` · Machine B
...
### Unit 9 — `scripts/newer_release_diff.py`  `NEW` · Machine C
...
### Unit 10 — `scripts/literature_curate.py`  `NEW` + manual · Machine C
...
# STAGE 4 — Figures & tables (Machine C)

### Unit 11 — `scripts/make_figures.py`  `NEW` · Machine C
...
### Unit 12 — `scripts/make_tables.py`  `NEW` · Machine C
...
# STAGE 5 — Release & manuscript (Machine C)

### Unit 13 — `run_all.ps1` (+ `run_all.sh`) and Zenodo  `NEW` · Machine C
...
guaranteed track) and BMC Bioinformatics / PLOS ONE (primary).

---

## The single ordered checklist (print this)

- [ ] 1. `graph_stats.py` → graph_stats.json  *(C)*
- [ ] 2. `build_deleaked_splits.py` real run → splits/ + hashes + upload  *(C)* ✅ built
- [ ] 3. provenance versions/dates/licenses filled  *(C)*
- [ ] 4. `lib_eval.py` + Random-control passes  *(C)*  ← A & B blocked until this lands
...
- [ ] 8. `loso_validation.py` → precision@k  *(B)*
- [ ] 9. `newer_release_diff.py` → recovery + mondo_labels.tsv → re-run Unit 2 with R3 filter  *(C)*
- [ ] 10. `literature_curate.py` → ≥5 confirmed novel candidates  *(C)*
- [ ] 11. `make_figures.py` → fig1–3  *(C)*
- [ ] 12. `make_tables.py` → tables 1–4  *(C)*
- [ ] 13. `run_all.ps1` + Zenodo DOIs  *(C)*
- [ ] 14. manuscript → preprint → submit  *(C)*

*Next action: Machine C runs Unit 2 for real (drop `--dry-run`), uploads `splits/`, then builds
Unit 1 (`graph_stats.py`) and Unit 4 (`lib_eval.py`). Units 5 and 6 start the moment `lib_eval.py`
passes its Random-control self-test.*
